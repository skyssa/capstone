...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    global $conn; // Replace with your actual database connection file
    ...
    $query = $conn->prepare('UPDATE `users` SET `school_id`=?,`fullname`=? ,`gender`=?,`email`=?,`address`=?,`number`=?,`acadyr`=?,`role_in_school`=?,`department`=?,`pic`=?,`cover`=?,`date_registered`=now(),`status`="active",`approval`="pending" WHERE `uid`=?');
    $query->bind_param('isssssisssss', $school_id, $fullname, $gender,$email, $address, $number, $acadyr, $role_in_school, $department, $pic, $cover,$user_id);
