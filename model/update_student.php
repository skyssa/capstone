...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = $conn->prepare('UPDATE users SET school_id=?,fullname=?,gender=?,email=?,`address`=?,`number`=?,yr_sec=?, acadyr=?,role_in_school=?,department=?,pic=?,cover=?,date_registered=now(),`status`="approval",approval="pending" WHERE `uid`=?');
    $query->bind_param('issssissssssi',$school_id,$fullname,$gender,$email,$address,$number,$yrsec,$acadyr,$role_in_school,$department,$pic,$cover,$id);
